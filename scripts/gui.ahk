...
MainGui.AddText("x1 y280 w448 h20 Right", "- Made by epic - v1.0.8")
...
Start(*) {
    KeyboardLayout()
    PlayerStatus("Starting v1.0.8 VicHopMacro by epic", "0xFFFF00", ,false, ,false)
    GetServerIds()
...
KeyboardLayout() {
    ...
    if (!language = 0x0409)
    {
        MsgBox("You may continue if the '/' key is working (And you have to press shift + key to open roblox chat) switch to US querty keyboard!. The macro WILL not work without the '/' key working.")
    }
}
